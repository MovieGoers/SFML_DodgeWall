...
void World::MakeWall() {
	...
	temp_rect.setPosition(rand() % (WALL_BOUNDARY_RIGHT - WALL_BOUNDARY_LEFT) + WALL_BOUNDARY_LEFT, 0.f);
...
void World::Update(float dt) {
	...
	for (int i = 0; i < walls_size; i++) {
		...
		if (tempWallPosition.y > WINDOW_HEIGHT) {
			...
			_wallSpeed += WALL_SPEED_INCREMENT;
			_wallSpawnTime -= WALL_RESPAWN_TIME_DECREMENT;
		}	
		else
...
std::vector<sf::RectangleShape> World::GetWalls() {
	return _walls;
}